...
class SkuGenerator
{
    ...
    public static function skuExists(string $sku): bool
    {
        ...
        $existsInPos = false;
        try {
            $existsInPos = DB::connection('boutique_pos')->table('items')->where('sku', $sku)->exists();
        } catch (\Exception $e) {
    ...
    private static function getNextSequenceNumber(string $categoryCode): int
    {
        ...
        $posSkus = [];
        try {
            $posSkus = DB::connection('boutique_pos')
                ->table('items')
        ...
        $allSkus = array_unique(array_merge($bodegaSkus, $posSkus));
        $existingNumbers = [];
        ...
        // Fill gaps randomly if under 9999, else increment
        for ($i = 0; $i < 1000; $i++) {
